# _*_coding:utf-8_*_
'''
Fork of https://github.com/Lynten/stanford-corenlp with fix
to add timeout parameter to java server init.

Also remove subprocess from member variable and only store
the pid. This allows the object to be serialised.
'''
...
class StanfordCoreNLP:
    def __init__(self, path_or_host, port=None, memory='4g', lang='en', timeout=1500, quiet=True,
        ...
        if path_or_host.startswith('http'):
            ...
            args = [cmd, java_args, '-cp', class_path, java_class,
                    '-port', str(self.port), '-timeout', str(self.timeout)]
            ...
            with open(os.devnull, 'w') as null_file:
                ...
                self.pid = p.pid
                logging.info('Server shell PID: {}'.format(self.pid))
    ...
    def close(self):
        ...
        if hasattr(self, 'pid'):
            try:
                ...
                logging.info('No process: {}'.format(self.pid))
                return
    ...
    def tregex(self, sentence, pattern):
        ...
        r_dict = self._request(tregex_url, pattern,
                               "tokenize,ssplit,depparse,parse", sentence)
        return r_dict

    def tokensregex(self, sentence, pattern):
        tokensregex_url = self.url + '/tokensregex'
        r_dict = self._request(tokensregex_url, pattern,
                               "tokenize,ssplit,depparse", sentence)
        return r_dict

    def semgrex(self, sentence, pattern):
        semgrex_url = self.url + '/semgrex'
        r_dict = self._request(semgrex_url, pattern,
                               "tokenize,ssplit,depparse", sentence)
        return r_dict
